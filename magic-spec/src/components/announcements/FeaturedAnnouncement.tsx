...
export const FeaturedAnnouncement = ({
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
      <div
        className={`px-4 py-3 flex items-center ${getTypeStyles(type).headerBg}`}>

        <span className="mr-2 text-xl">{getTypeEmoji(type)}</span>
        <span
          className={`text-sm font-medium ${getTypeStyles(type).headerText}`}>
      ...
      <div className="p-6">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-2/5 mb-4 md:mb-0 md:mr-6">
              ...
              src={image}
              alt={title}
              className="w-full h-64 md:h-56 object-cover rounded-md shadow-sm hover:opacity-90 transition-opacity" />
...
const getTypeStyles = (type) => {
  switch (type) {
    ...
    default:
      return {
        headerBg:
        'bg-news-primary bg-opacity-10 border-l-4 border-news-primary',
