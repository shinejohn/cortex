...
class RawContent extends Model
{
    ...
    public function collectionMethod() { return $this->belongsTo(CollectionMethod::class, 'collection_method_id'); }
    public function community() { return $this->belongsTo(Community::class); }
    public function article() { return $this->belongsTo(NewsArticleDraft::class, 'article_id'); }
    public function businessMentions() { return $this->hasMany(BusinessMention::class, 'raw_content_id'); }

    public function scopePendingClassification($q) { return $q->where('classification_status', self::CLASS_PENDING); }
    public function scopeClassified($q) { return $q->where('classification_status', self::CLASS_CLASSIFIED); }
    ...
    public function scopeByTier($q, $t) { return $q->where('processing_tier', $t); }
    public function scopeBreaking($q) { return $q->where('priority', self::PRIORITY_BREAKING); }
    public function scopeHasEvent($q) { return $q->where('has_event', true); }

    public static function generateContentHash(string $title, ?string $url = null): string {
        return hash('sha256', trim(strtolower($title)) . '|' . ($url ?? ''));
    }

    public static function generateTitleHash(string $title): string {
        return hash('sha256', trim(strtolower($title)));
    }

    public static function isDuplicate(string $hash, int $communityId): bool {
