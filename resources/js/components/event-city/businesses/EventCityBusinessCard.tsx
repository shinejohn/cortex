...
export function EventCityBusinessCard({ business, upcomingEventsCount = 0, nextEvent, className }: EventCityBusinessCardProps) {
    ...
    return (
        <div
            className={cn(
                "group relative overflow-hidden rounded-xl border border-indigo-200 bg-gradient-to-br from-white to-indigo-50/30 p-5 shadow-lg transition-all hover:border-indigo-400 hover:shadow-xl",
                className,
            )}
        >
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                        <Link href={href} className="group/link">
                            <h3 className="text-xl font-bold text-indigo-900 group-hover/link:text-indigo-700">{business.name}</h3>
                        </Link>
                        {business.is_verified && (
                            <span className="ml-2 rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-700">✓ Verified Venue</span>
                        )}
                    </div>
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-semibold text-indigo-900">{nextEvent.title}</span>
                                </div>
