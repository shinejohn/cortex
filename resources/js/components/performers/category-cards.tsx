...
export function CategoryCards({ categories = defaultCategories, className }: CategoryCardsProps) {
    return (
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-4">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Discover performers across different musical styles</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {categories.map((category) => {
                        ...
                        return (
                            <Link key={category.id} href={`/performers?category=${category.id}`} className="group block">
                                <div
                                    className={cn(
                                ...
                                >
                                    <div className="flex items-center justify-between mb-4">
                                        <div className={cn("p-3 rounded-lg", "bg-white dark:bg-slate-800/50")}>
                                            <IconComponent className={cn("h-6 w-6", colors.icon)} />
                                        </div>
                                        <ArrowRightIcon className={cn("h-5 w-5 transition-transform group-hover:translate-x-1", colors.text)} />
                                    </div>

                                    <div>
                                        <h3 className={cn("text-lg font-semibold mb-1", colors.text)}>{category.name}</h3>
                                        <p className="text-sm text-muted-foreground">{category.count} performers available</p>
                                    </div>
