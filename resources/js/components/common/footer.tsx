        ...
        <div className="flex flex-wrap space-x-4 mt-2">
            {LEGAL_LINKS.map((link, index) => (
                <span key={link.href} className="flex items-center">
                    ...
                    {index < LEGAL_LINKS.length - 1 && <span className="text-muted-foreground/50 ml-4">|</span>}
                </span>
            ))}
        </div>
    );
    ...
    return (
        <div className="flex flex-col sm:flex-row gap-2">
            {APP_STORE_LINKS.map((app) => (
                <Button key={app.name} variant="secondary" size="sm" className="text-xs transition-colors duration-200" asChild>
