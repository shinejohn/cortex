...
Yes, there **are** exploitable patterns across all four platforms. Here's what's available:
...
| **Granicus MediaManager** | No | **YES** | **YES** | `{city}.granicus.com` + probe |
...
## 3. Granicus MediaManager / govMeetings - RSS Feeds

### This is SEPARATE from Legistar!

Granicus also has a **MediaManager/govMeetings** platform for meeting videos, agendas, and minutes. This uses RSS feeds, not a REST API.

### URL Structure
```
# Base Host (subdomain pattern)
https://{city}.granicus.com
https://{region}.granicus.com

# Publisher Endpoints (HTML listings)
/ViewPublisher.php?view_id={ID}    # Meeting channel listing
...
1. **Find Granicus Host from City Website**
   ```bash
...
2. **Enumerate Publishers (view_ids)**
   ```bash
   # Brute-force view_id 1-50
   for id in {1..50}; do
     curl -s "https://{city}.granicus.com/ViewPublisher.php?view_id=$id"
   done
   
   # Valid pages return meeting lists
   # Invalid pages return error/empty
   ```

3. **Find RSS Feeds for Each Publisher**
   ```bash
   # For each valid view_id, try:
   curl "https://{city}.granicus.com/xml/MediaRSS.php?view_id={ID}"
   curl "https://{city}.granicus.com/boards/rss/{ID}"
   
   # Also scan the publisher page for:
...
```bash
# Publisher page
http://dunedin.granicus.com/ViewPublisher.php?view_id=6

# RSS feed attempts
http://dunedin.granicus.com/xml/MediaRSS.php?view_id=6
http://dunedin.granicus.com/boards/rss/6
```

### Common Publisher Types (view_ids)
Most cities have multiple channels:
...
MediaPlayer.php
govMeetings
<!-- OR script/image patterns -->
/Granicus/
```

### Key Difference from Legistar
| Legistar | Granicus MediaManager |
|----------|----------------------|
| REST API | RSS Feeds |
| Legislation tracking | Meeting videos |
| Votes, actions | Agendas, minutes |
| webapi.legistar.com | {city}.granicus.com |

Many cities use BOTH - Legistar for legislative tracking AND Granicus MediaManager for video streaming.

---

## 4. Nixle - Predictable URL Patterns
