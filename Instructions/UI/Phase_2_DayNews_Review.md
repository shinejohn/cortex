...
**ALL issues are fixed. No exceptions. No "fix if time permits." A hundred small issues will wreck a deployment just as fast as one critical bug.**

| Level | Definition | Action |
|-------|-----------|--------|
| **CRITICAL** | Page crashes, 500 error, data not loading, complete feature broken | Fix immediately upon discovery, do not continue reviewing until fixed |
| **HIGH** | Button doesn't work, link goes nowhere, form doesn't submit, "undefined" shown to user | Fix immediately upon discovery, do not continue reviewing until fixed |
| **MEDIUM** | Styling issue, minor layout problem, missing loading state, inconsistent spacing, lint warning | Fix during the same phase. Must be resolved before phase sign-off |
| **LOW** | Unused import, minor code style, optimization opportunity, console.log left in code | Fix during the same phase. Must be resolved before phase sign-off |

**The rule is simple: every phase ends with zero open issues across all severity levels. If Antigravity finds 400 low-severity issues, Antigravity fixes 400 low-severity issues. These are exactly the kind of issues that cause npm run build to spit out 200 warnings, ESLint to choke, TypeScript to throw errors, and Railway deployments to fail silently. Fix them all.**
